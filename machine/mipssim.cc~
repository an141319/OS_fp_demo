...
Machine::Run()
{
    ...
    kernel->interrupt->setStatus(UserMode); // change system state to UserMode
    for (;;) {
        OneInstruction(instr);	// exec instruction
	kernel->interrupt->OneTick(); // push time forward a unit
...
Machine::OneInstruction(Instruction *instr)
{
    ...
    // Fetch instruction 
    if (!ReadMem(registers[PCReg], 4, &raw))	// stored read val in raw
